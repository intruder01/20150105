...
namespace OpenHTM.CLA
{
	...
	public class DistalSynapse : Synapse
	{
		...
		{
			return "DistSynapse + (Cell[[" + DistalSegment.parentCell.Column.PositionInRegion.X + "," + DistalSegment.parentCell.Column.PositionInRegion.Y + "]" + DistalSegment.parentCell.Index +"])" 
					+ " Src=" + this.InputSource.ToString() + (this.IsActive(Global.T) ? " A" : " ")
