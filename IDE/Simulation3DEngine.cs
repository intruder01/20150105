	...
	internal class Simulation3DEngine : Game
	{
		...
		protected override void LoadContent()
		{
			if (!this._contentLoaded)
			{
				...
				this._whiteTexture = LoadTexture2D ( this.GraphicsDevice, "./Content/Simulation3DWhiteTexture.png" );
				...
				this._dictionaryProximalSynapseColors.Add ( HtmProximalSynapseColors.ActiveConnected, new HtmColorInformation ( Color.LimeGreen, "Proximal synapse is active and connected" ) );
				...
				this._dictionaryDistalSynapseColors.Add ( HtmDistalSynapseColors.Active, new HtmColorInformation ( Color.Black, "Distal synapse active" ) );
		...
		/// <param name="mapData"></param>
		/// <param name="title"> </param>
		private void DrawHtmInputPredictionReconstructionMap(Vector2 startVectorLeft, float[,] mapData, string title) //20160109-1
		{
			// Count active elements
			int activeCounter = 0;
			var gridLeftStart = (int) startVectorLeft.X;
			...
			string activeColumnsString = "Predicted Inputs: " + activeCounter.ToString();
			startVectorLeft.Y += _gridHeight + _gridHeightBuffer;
			this._spriteBatch.DrawString(this._spriteFont, activeColumnsString, startVectorLeft,Color.Black);
		...
		private void DrawHtmInputPlane()
		{
			...
			{
				// occasionally data is in transition and causes exception, abort draw when that is the case 
				return;
			}
		}
